...
import {OTPService} from './otpServivce.js';
...
export default class OTP extends React.Component {
  ...
  componentWillMount() { 
        ...
        let otpData = JSON.parse(localStorage.getItem('userData'));
        if (!otpData ||
            !otpData.data ||
            !otpData.data.time ||
  ...
  render() {
      return (
      <OTPView
      setOTP={(otp) => this.setState({otp:otp.target.value})}
